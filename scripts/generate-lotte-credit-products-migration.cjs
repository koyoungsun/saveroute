#!/usr/bin/env node
/**
 * 롯데카드 신용 목록(JSON) → supabase 마이그레이션 benefit_products INSERT (멱등)
 *
 *   node scripts/generate-lotte-credit-products-migration.cjs
 *
 * 입력: scripts/data/lotte_credit_cards.input.json
 * 출력: supabase/migrations/053_seed_lotte_credit_card_products.sql
 */
...
const INPUT = path.join(ROOT, "scripts", "data", "lotte_credit_cards.input.json");
const OUT = path.join(ROOT, "supabase", "migrations", "053_seed_lotte_credit_card_products.sql");
...
function deriveCode(name, usedCodes) {
  const salt = `lotte_card|credit|${name}`;
  const frag = asciiSlugFragment(name);
  let tail = frag.length >= 4 ? frag : `x${hash12(salt)}`;
  let code = `lotte_cred_${tail}`.slice(0, 100);
  if (usedCodes.has(code)) code = `lotte_cred_${hash12(`${name}|credit`)}`;
  let dup = 2;
  while (usedCodes.has(code)) {
    code = `lotte_cred_${hash12(`${name}:${dup}`)}`;
    dup += 1;
  }
  usedCodes.add(code);
  return code;
...
function main() {
  ...
  const sql = `-- 롯데카드 신용 benefit_products 멱등 시드
